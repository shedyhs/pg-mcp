...
export async function handleGetDdl(args) {
...
async function extractEnums(pool, schemaFilter, ddlParts) {
...
async function extractSequences(pool, schema, ddlParts) {
    const result = await pool.query(`
  ...
  `);
    for (const seq of result.rows) {
        ...
        ddlParts.push(`CREATE SEQUENCE ${seq.schema}.${seq.name} START ${seq.start_value} INCREMENT ${seq.increment_by} MINVALUE ${seq.min_value} MAXVALUE ${seq.max_value}${seq.cycle ? " CYCLE" : ""};\n`);
    }
}
async function extractTables(pool, schemaFilter, ddlParts) {
  ...
  `);
    ...
    `, [table.table_oid]);
        ...
        ddlParts.push(`CREATE TABLE ${table.schema}.${table.table_name} (\n${columns.join(",\n")}\n);\n`);
    }
}
async function extractForeignKeys(pool, schema, ddlParts) {
    const result = await pool.query(`
    SELECT
  ...
  `);
    for (const fk of result.rows) {
        ...
        ddlParts.push(`ALTER TABLE ${fk.schema}.${fk.table_name} ADD CONSTRAINT ${fk.constraint_name} FOREIGN KEY (${fk.column_name}) REFERENCES ${fk.foreign_schema}.${fk.foreign_table}(${fk.foreign_column}) ON UPDATE ${fk.update_rule} ON DELETE ${fk.delete_rule};\n`);
    }
}
async function extractUniqueConstraints(pool, schemaFilter, ddlParts) {
    const result = await pool.query(`
    SELECT
      n.nspname AS schema,
  ...
  `);
    for (const uq of result.rows) {
        ...
        ddlParts.push(`ALTER TABLE ${uq.schema}.${uq.table_name} ADD CONSTRAINT ${uq.constraint_name} UNIQUE (${cols});\n`);
    }
}
async function extractIndexes(pool, schemaFilter, ddlParts) {
    const result = await pool.query(`
    SELECT
      n.nspname AS schema,
      t.relname AS table_name,
...
async function extractViews(pool, schema, ddlParts) {
    const result = await pool.query(`
    SELECT schemaname AS schema,
           viewname AS name,
           definition
